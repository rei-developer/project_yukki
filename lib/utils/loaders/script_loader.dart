...
class ScriptLoader {
  ...
  Future<List<ScriptModel>> loadLocalScripts() async {
    ...
    final localScripts = (await StorageManager(scriptsPath).files)?.where((e) => e.endsWith(jsExt)).toList();
    ...
    for (final localScript in localScripts) {
      ...
      final fileName = localScript.replaceAll('$documents/$scriptsPath/', '').replaceAll(jsExt, '');
  ...
  Future<List<ScriptModel>> _loadScriptsAssets(String type, String source, String assetsPath) async {
    List<ScriptModel> scripts = [];
    ...
    for (final key in keys) {
      ...
      scripts.add(
        ScriptModel(
          type,
          fileName,
          await AssetManager(assetsPath, fileName, jsExt).loadString,
        ),
      );
    }
    return scripts;
  ...
  String get _defaultScriptsSource => r'assets\/scripts\/default_scripts\/(.*?)\.js';
}
